...
const GoalForm = ({ hasEdit, goalDefault }: GoalFormProps) => {
  ...
  const { mutate: updateGoalMutation } = api.goals.update.useMutation();
  ...
  useEffect(() => {
    if (goalDefault) {
      ...
      const { formatted: savedFormatted, raw: rawSaved } = amountFormatter(
        String(goalDefault.saved),
      );

      setGoalAmount(goalFormatted);
      setInitialAmount(savedFormatted);
      setValue("goal", rawGoal!);
      setValue("saved", rawSaved!);
      ...
      >
        ...
        <div className="flex gap-2 ">
          ...
          <Input
            ...
            isDisabled={Boolean(goalDefault)}
            placeholder="0.00"
            className="!appearance-none"
