...
import { BASIC_DATE, DATE_FORMAT_TRANS, DEFAULT_DATES, TODAY_DATE } from '~/lib/constants/config';
...
export const useTransactionSeries = (transactions: Transaction[]) => {
  ...
  const setChartSeries = () => {
    ...
    const firstDate = new Date(myTransaction[0]!.date ?? myTransaction[0]!.createdAt);
    const lastDate = new Date(myTransaction[myTransaction.length - 1]!.date ?? myTransaction[myTransaction.length - 1]!.createdAt);
    ...
    if (getMonth(firstDate) === getMonth(lastDate) && getYear(firstDate) === getYear(lastDate)) {
      granularity = 'week';
      // si las transacciones son del misma semana
      if (getWeek(firstDate) === getWeek(lastDate) && getDay(firstDate) === getDay(lastDate) && getYear(firstDate) === getYear(lastDate)) {
        granularity = 'day';
      }
    }

    const groupedTransactions = _.groupBy(myTransaction, (tran: Transaction) => {
    ...
    const sortedGroups = granularity === "week" ? Object.keys(groupedTransactions) : Object.keys(groupedTransactions).sort();
    ...
    for (const group of sortedGroups) {
      ...
      for (const tran of groupTransactions!) {
        if (tran.type === 1 && tran.transferType === 1 || tran.type === 2 && tran.transferType === 2) {
