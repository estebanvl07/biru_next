    ...
    <Card className="flex flex-col h-full">
      ...
      <nav className="scrollbar-customize flex w-full gap-x-2 overflow-auto py-4">
        ...
        >
          ...
          <span className="w-12 text-xs text-center text-ellipsis mt-0.5 whitespace-nowrap overflow-hidden">Crear</span>
        </div>
          ...
          return (
            ...
            <div key={id} className="flex flex-col items-center justify-center">
              <Avatar
                ...
                name={name}
                color="primary"
                onClick={() => setEntitySelected((prev) => prev === id ? undefined : id)}
                classNames={{
              ...
              />
              <span className="w-12 text-xs text-center text-ellipsis mt-0.5 whitespace-nowrap overflow-hidden">{name}</span>
            </div>
            // </Link>
          );
        })}
      ...
      <form className="w-full flex flex-col gap-2" onSubmit={onSubmit}>
        ...
        />
        <Button color="primary" type="submit" isDisabled={!Boolean(amount)}>Crear Transacción</Button>
      </form>
    </Card>
  );
};
